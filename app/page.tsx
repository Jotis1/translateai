...
export default function Page() {
  ...
  const [stateAuth, formAuth] = useFormState(AuthUser, initialState);
  ...
  return (
    <main className='w-screen font-medium h-screen py-5 px-10 text-zinc-950 text-center flex flex-col gap-10 justify-center'>
      ...
      {!access ? (
        <form action={formAuth} className='p-5 text-zinc-950 flex flex-col border-2 border-zinc-300 rounded-lg gap-6'>
          <section className='flex flex-col items-start justify-center w-full gap-2'>
            <label className='text-zinc-700' htmlFor="user">Usuario</label>
            <input name='name' className='w-full border-2 h-10 px-2.5 rounded-lg border-zinc-300' placeholder='Introduce tu usuario' type="text" required />
          </section>
          <section className='flex flex-col items-start justify-center w-full gap-2'>
            <label className='text-zinc-700' htmlFor="password">Contraseña</label>
            <input name='password' className='w-full border-2 h-10 px-2.5 rounded-lg border-zinc-300' placeholder='Introduce tu usuario' type="password" required />
          </section>
